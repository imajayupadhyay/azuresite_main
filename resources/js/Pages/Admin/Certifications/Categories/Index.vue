<template>
    <div class="min-h-screen bg-gray-50">
        <Sidebar />
        <Topbar title="Certification Categories" subtitle="Manage certification categories" />
...
const applyFilters = () => {
    router.get(route('admin.certifications.categories.index'), filterForm, {
        preserveState: true,
        preserveScroll: true
    });
};
...
const saveCategory = () => {
    ...
    if (editingCategory.value) {
        ...
        router.put(route('admin.certifications.categories.update', editingCategory.value.id), form, {
            preserveScroll: true,
            onSuccess: () => {
                closeModal();
            },
        ...
        // Create new category
        router.post(route('admin.certifications.categories.store'), form, {
            preserveScroll: true,
            onSuccess: () => {
                closeModal();
            },
...
const toggleStatus = (category) => {
    if (confirm(`Are you sure you want to ${category.is_active ? 'deactivate' : 'activate'} "${category.name}"?`)) {
        router.put(route('admin.certifications.categories.toggle-status', category.id), {}, {
            preserveScroll: true
        });
    }
};
...
const deleteCategory = (category) => {
    if (confirm(`Are you sure you want to delete "${category.name}"? This action cannot be undone.`)) {
        router.delete(route('admin.certifications.categories.destroy', category.id));
    }
};
</script>
