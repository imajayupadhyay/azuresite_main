...
const form = ref({
    ...
    icon_identifier: props.service.icon_identifier || '',
    video_url: props.service.video_url || '',
    video_thumbnail: props.service.video_thumbnail || '',
    video_duration: props.service.video_duration || '',
    difficulty: props.service.difficulty || 'beginner',
    order: props.service.order || 0,
    is_active: props.service.is_active ?? true,
    tutorial_sections: (props.service.tutorial_sections || []).map(section => ({
        ...section,
...
const removeSection = (index) => {
    const section = form.value.tutorial_sections[index];
    if (section.id) {
        form.value.deleted_sections.push(section.id);
    }
...
const removeContentBlock = (sectionIndex, blockIndex) => {
    const block = form.value.tutorial_sections[sectionIndex].content_blocks[blockIndex];
    if (block.id) {
        form.value.deleted_blocks.push(block.id);
    }
...
const expandedSections = ref(new Set(form.value.tutorial_sections.map((_, i) => i)));
...
const submit = () => {
    ...
    // Add tutorial sections as JSON
    form.value.tutorial_sections.forEach((section, sIndex) => {
        formData.append(`tutorial_sections[${sIndex}][id]`, section.id || '');
        ...
        section.content_blocks.forEach((block, bIndex) => {
            formData.append(`tutorial_sections[${sIndex}][content_blocks][${bIndex}][id]`, block.id || '');
    ...
    // Add deleted sections and blocks
    form.value.deleted_sections.forEach((id, index) => {
        formData.append(`deleted_sections[${index}]`, id);
    });
    form.value.deleted_blocks.forEach((id, index) => {
        formData.append(`deleted_blocks[${index}]`, id);
    });

    router.post(route('admin.services.update', props.service.id), formData, {
        forceFormData: true,
        onSuccess: () => {
...
<template>
    <div class="min-h-screen bg-gradient-to-br from-slate-50 via-white to-blue-50">
        <Sidebar />
        <Topbar title="Edit Service" :subtitle="`Editing: ${service.name}`" />
                    ...
                    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 mb-6">
                        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                            <div class="flex items-center gap-4">
                                <div class="h-14 w-14 rounded-xl bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center shadow-lg shadow-blue-500/25">
                                ...
                                <div>
                                    ...
                                    <p class="text-sm text-gray-500 mt-0.5">Update service details and tutorial content</p>
                                </div>
                            </div>
                            <div class="flex items-center gap-3">
                                ...
                                <button
                                    @click="submit"
                                    :disabled="processing"
                                    class="inline-flex items-center px-6 py-2.5 text-sm font-semibold text-white bg-gradient-to-r from-blue-600 to-indigo-600 rounded-xl hover:from-blue-700 hover:to-indigo-700 shadow-lg shadow-blue-500/25 transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
                                >
                            ...
                            <div class="p-6">
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    ...
                                    <div class="space-y-2">
                                        ...
                                        <div v-if="videoPreview || service.video_url" class="mt-3">
                                            <video :src="videoPreview || service.video_url" controls class="w-full max-h-48 rounded-xl"></video>
                                        </div>
                                    </div>
                                    ...
                                    <div class="space-y-2">
                                        ...
                                        <div v-if="thumbnailPreview || service.video_thumbnail" class="mt-3">
                                            <img :src="thumbnailPreview || service.video_thumbnail" alt="Thumbnail" class="w-full max-h-48 object-cover rounded-xl" />
                                        </div>
                                    </div>
                            ...
                            <div class="flex items-center justify-between">
                                ...
                                <div class="flex items-center gap-3">
                                    ...
                                    <button
                                        type="submit"
                                        :disabled="processing"
                                        class="px-6 py-2.5 text-sm font-semibold text-white bg-gradient-to-r from-blue-600 to-indigo-600 rounded-xl hover:from-blue-700 hover:to-indigo-700 shadow-lg shadow-blue-500/25 transition-all disabled:opacity-50"
                                    >
                                        {{ processing ? 'Updating...' : 'Update Service' }}
                                    </button>
                                </div>
